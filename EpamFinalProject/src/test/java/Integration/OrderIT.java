...
class OrderIT {
  ...
  void createOrderTest() {
    Order order = new Order(1, cruise, user, Status.PENDING);
  ...
  void payForOrderTest() {
    Order order = new Order(1, cruise, user, Status.PAID);
    orderService.createOrder(order);
  ...
  void getBookedSeatsTest() {
    ...
    orderService.createOrder(order);
    assertThat(
            orderService.getBookedSeatsByCruiseID(orderService.getOrdersByUserID(1).get(0).getId()))
        .isEqualTo(2);
  }
  @Test
  void confirmOrderTest(){
    Order order = new Order(1, cruise, user, Status.CONFIRMED);
    orderService.createOrder(order);
