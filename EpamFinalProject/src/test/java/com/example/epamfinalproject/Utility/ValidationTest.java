...
class ValidationTest {
    ...
    void validateUserFieldsTrueTest(String login, String firstName, String lastName) {
        assertTrue(Validation.validateUserFields(new User.UserBuilder().login(login).firstName(firstName).lastName(lastName).build()));
    }
    @ParameterizedTest
    @MethodSource("invalidUserData")
    void validateUserFieldsFalseTest(String login, String firstName, String lastName) {
        assertFalse(Validation.validateUserFields(new User.UserBuilder().login(login).firstName(firstName).lastName(lastName).build()));
    }
    ...
    void validateCruiseTrueFields(String name) {
        assertTrue(Validation.validateCruiseFields(new Cruise(new Ship(), new Route(),name,0,null,null)));
    }
 @ParameterizedTest
    @MethodSource("invalidCruiseData")
    void validateCruiseFalseFields(String name) {
        assertFalse(Validation.validateCruiseFields(new Cruise(new Ship(), new Route(),name,0,null,null)));
    }
    ...
    static Stream<Arguments> validDateData() {
        return Stream.of(
                Arguments.of("2002-12-12"),
                Arguments.of("2033-12-30"),
                Arguments.of("1000-10-11"));
    }
    ...
    static Stream<Arguments> invalidCruiseData() {
        return Stream.of(
